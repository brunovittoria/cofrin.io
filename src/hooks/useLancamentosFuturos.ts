...
export const useLancamentosFuturos = (dateRange?: DateRange, status?: 'pendente' | 'efetivado') => {
  return useQuery({
    queryKey: ['lancamentos-futuros', dateRange?.from?.toISOString(), dateRange?.to?.toISOString(), status],
    queryFn: async () => {
      let query = supabase
      ...
      return data as (LancamentoFuturo & { categorias?: { nome: string; cor_hex?: string } })[];
    },
  });
};
...
export const useLancamentosFuturosSummary = (dateRange?: DateRange) => {
  return useQuery({
    queryKey: ['lancamentos-futuros-summary', dateRange?.from?.toISOString(), dateRange?.to?.toISOString()],
    queryFn: async () => {
      let query = supabase
        .from('lancamentos_futuros')
...
export const useCreateLancamentoFuturo = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['lancamentos-futuros-summary'] });
      toast({
...
export const useUpdateLancamentoFuturo = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, ...updates }: Partial<LancamentoFuturo> & { id: number }) => {
      const { data, error } = await supabase
        .from('lancamentos_futuros')
        .update(updates)
        .eq('id', id)
