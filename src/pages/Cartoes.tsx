...
import { useCartoes, useDeleteCartao, useSetCartaoPrincipal } from "@/hooks/useCartoes";
        ...
        <section className="muted-card p-6 sm:p-8">
          <header className="flex flex-col gap-4 border-b border-[#E5E7EB] pb-6 sm:flex-row sm:items-center sm:justify-between">
            <div className="space-y-2">
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-[#94A3B8]">Cartoes</p>
              <div className="space-y-1">
                <h1 className="text-3xl font-semibold text-[#0F172A]">Gestao de Cartoes</h1>
                <p className="text-sm text-[#4B5563]">
                  Visualize os principais cartoes e deixe pronto o espaco para inserir as imagens oficiais.
                </p>
              </div>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-3">
              {cardsToDisplay.map((card) => {
                const percentual = card.uso_percentual ?? (card.limite_total > 0 ? Number((card.valor_utilizado / card.limite_total) * 100) : 0);
                const available = card.valor_disponivel ?? card.limite_total - card.valor_utilizado;
                const { label: statusLabel, className: statusClass } = getStatusStyles(percentual);
                ...
                return (
                  ...
                  >
                    {imageUrl ? (
                      <img src={imageUrl} alt={provider?.name ?? card.nome_exibicao} className="object-cover w-[65%] h-[65%] mx-auto my-4" />
                    ) : (
                    ...
                    <div className="space-y-5">
                      <div className="flex items-start justify-between gap-4">
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            <span
                              className={cn(
                                ...
                                brandStyles[card.bandeira as keyof typeof brandStyles]?.badge ?? "bg-[#E2E8F0] text-[#475569]",
                              )}
                            >
                          ...
                          <h2 className="text-lg font-semibold text-[#0F172A]">{card.nome_exibicao}</h2>
                          {/* Removido Banco e Alias */}
                        </div>
                        <span className={cn("rounded-full px-3 py-1 text-xs font-semibold", statusClass)}>{statusLabel}</span>
                      </div>

                      <div className="grid gap-4 sm:grid-cols-3">
                        <div>
                          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#94A3B8]">Limite</p>
                          <p className="mt-1 text-base font-semibold text-[#0F172A]">
                            R$ {card.limite_total.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                          </p>
                        </div>
                        <div>
                          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#94A3B8]">Utilizado</p>
                          <p className="mt-1 text-base font-semibold text-[#0F172A]">
                            R$ {card.valor_utilizado.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                          </p>
                        </div>
                        <div>
                          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#94A3B8]">Disponivel</p>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between text-xs text-[#6B7280]">
                          ...
                          <span className={percentual > 80 ? "font-semibold text-[#DC2626]" : "font-semibold text-[#0F172A]"}>
                    ...
                    <div className="flex gap-2 mt-2">
                      ...
                      >
                        <Star className={`h-4 w-4 ${card.is_principal ? "text-yellow-500 fill-yellow-500" : "text-gray-500"}`} />
                      </Button>
                      <Button
                        size="sm"
                        variant="destructive"
