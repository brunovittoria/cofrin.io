...
import { useIncomeExpenseData } from "@/hooks/api/useChartData";
...
export function IncomeExpenseChart({ dateRange }: IncomeExpenseChartProps) {
  ...
  if (isLoading) {
    return (
      <section className="rounded-3xl border border-border bg-card p-6 shadow-sm transition-colors sm:p-7">
        {renderHeader()}
        <Skeleton className="mt-6 h-[340px] w-full bg-muted" />
      </section>
    );
  }
  ...
  if (error || !data) {
    return (
      <section className="rounded-3xl border border-border bg-card p-6 shadow-sm transition-colors sm:p-7">
        {renderHeader()}
        <div className="flex h-[340px] items-center justify-center text-sm font-medium text-muted-foreground">
      ...
      <div className="mt-6 h-[340px]">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Bar
              dataKey="entradas"
              fill="url(#entradas-gradient)"
              radius={[8, 8, 0, 0]}
              name="Entradas"
            />
            <Bar
              dataKey="saidas"
              fill="url(#saidas-gradient)"
              radius={[8, 8, 0, 0]}
              name="Saidas"
            />
          </BarChart>
